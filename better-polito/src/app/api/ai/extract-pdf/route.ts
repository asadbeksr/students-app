...
import { renderPageWithMarker, type PdfParseOptions } from './markerRenderer';

// eslint-disable-next-line @typescript-eslint/no-require-imports
const pdfParse: (buffer: Buffer, options?: PdfParseOptions) => Promise<{ text: string; numpages: number }> = require('pdf-parse');
...
async function extractTextFromPdf(buffer: ArrayBuffer): Promise<{ text: string; pageCount: number; isLikelyScanned: boolean }> {
  const data = await pdfParse(Buffer.from(buffer), { max: 50, pagerender: renderPageWithMarker });
