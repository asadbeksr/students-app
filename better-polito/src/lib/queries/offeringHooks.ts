...
export const useGetOffering = () => useQuery({
  ...
  queryFn: () => getApiClient().getOffering().then((r: any) => r.data),
  staleTime: Infinity,
});

export const useGetOfferingDegree = (degreeId: string, year?: number) => useQuery({
  queryKey: [DEGREES_QUERY_PREFIX, degreeId, year],
  queryFn: () => getApiClient().getOfferingDegree(degreeId, year).then((r: any) => r.data),
  enabled: !!degreeId,
});
...
export const useGetCourseStatistics = (shortcode: string, teacherId?: number, year?: number) => useQuery({
  ...
  queryFn: () => getApiClient().getCourseStatistics(shortcode, teacherId, year).then((r: any) => r.data),
  enabled: !!shortcode,
});
